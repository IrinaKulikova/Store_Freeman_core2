...
namespace Store.Tests
{
    ...
    {
        [Fact]
        public async void ToyController_WithMockRepositoryPage3_ShouldReturnPaginationInfo()
        {
            ToysListViewModel result = await GetViewModel();
        ...
        private async Task<ToysListViewModel> GetViewModel()
        {
            Mock<IToyRepository> mockToyRepository = new Mock<IToyRepository>();
            mockToyRepository.Setup(m => m.Toys()).ReturnsAsync((new List<Toy>()
            {
        ...
        [Fact]
        public async void ToyController_WithMockRepositoryPage3_ShouldReturnListToys()
        {
            ToysListViewModel result = await GetViewModel();
